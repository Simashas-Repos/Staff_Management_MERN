...
function Staff({ staff, index, hideActions }) {
  const { _id, image, name, address, gender, email, nic, jobRole, contactNo } =
    staff;
  ...
  return (
    <tr>
      ...
      <td>{jobRole}</td>
      <td>{contactNo}</td>
      <td className={!hideActions ? "print-hide" : ""}>
        <Link to={`/staffdetails/${_id}`} style={{ textDecoration: "none" }}>
          <button className="update1">Update</button>
        </Link>
        <button className="delete1" onClick={deleteHandler}>
          Delete
        </button>
      </td>
    </tr>
